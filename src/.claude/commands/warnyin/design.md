...
5. ตอน generate ไฟล์ task หลายใบ: **standard/large tier** → fan-out **default** หนึ่ง read-only agent ต่อหนึ่ง task ขนาน (หลังผ่าน Gate §8 ของ playbook); **fast tier** → 1 task เขียนเอง ไม่ fan-out — ดู playbook §4 step 9 + §7 tier table สำหรับรายละเอียด parallelization + fallback
