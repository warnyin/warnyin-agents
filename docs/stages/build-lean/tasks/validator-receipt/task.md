...
- [x] 1. **`src/.warnyin/workflow/scripts/validate-topic.mjs`** — เพิ่ม fast-mode detection — _ผลลัพธ์: 3 เส้นทางตาม contract §4.2_
...
- [x] 2. **`src/tests/validate-topic.test.mjs`** — เพิ่มเคสตาม design §9 scenario — _ขึ้นกับ 1:_
...
- [x] 3. **`src/.warnyin/workflow/next.md`** — ตาราง stage-inference (§2 ข้อ 3, บรรทัด ~24-31) — _ขึ้นกับ 1 (คำว่า `fast-track` ให้ตรงกับ output validator):_
...
- [x] `npm test` เขียว — เคสใหม่ (fast/mixed/template) + เคสเดิมทั้งหมดผ่าน โดย**ไม่แก้ assertion เคสเดิม**
- [x] pass รวม ≥ MIN_PASS 9 (`node src/scripts/check-test-count.mjs` / gate เดิม) — pass=121
- [x] exit codes เดิมคง: status = 0 เสมอ · validate มี ✖ = 1 · slug ผิด/arg เกิน = 2
- [x] zero-dependency คง — import เฉพาะ `node:*` (ไม่มี child_process/network/write ใน script)
- [x] fast detection **ไม่ทำ topic เดิมเปลี่ยนผล**: รัน status mode กับ repo จริง (`docs/stages/` + achieved) ก่อน/หลังแก้ → output ตารางเหมือนเดิม (build-lean = DESIGN ✖0/⚠0 ทั้งคู่)
- [x] mixed-state เป็น ⚠ เท่านั้น (ไม่ใช่ ✖ — ไม่ทำ exit 1 ด้วยตัวเอง)
- [x] `next.md` มี row fast-track ในตาราง stage-inference + ไฟล์ส่วนอื่นไม่เปลี่ยน
- [x] แก้เฉพาะ `src/**` (3 ไฟล์ตาม §4)
