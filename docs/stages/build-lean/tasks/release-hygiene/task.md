...
- [ ] 1. **`CHANGELOG.md`** — _ผลลัพธ์: entry ใหม่ตามรูปแบบเดิม (Keep a Changelog, หัวข้อ `## [x.y.z] - YYYY-MM-DD` + `### Added`/`### Changed`)_ สรุป user-facing changes ของ change ทั้งชุด (อ่านจากผล diff จริงของ wave 1-2 บน build branch ไม่ใช่เดาจาก design):
...
- [ ] 2. **`package.json`** — _ขึ้นกับ 1 (เลข version ต้องตรงหัว entry):_ bump → **`0.24.0`** (**minor** — มี behavior ใหม่ backward-compatible ไม่ breaking)
...
- [ ] CHANGELOG entry ครบทุก user-facing change (7 ข้อใน sub-task 1) — รูปแบบตรงไฟล์เดิม (heading + วันที่ + Added/Changed) และตรงกับ diff จริงบน build branch
- [ ] `package.json` version = `0.24.0` ตรงกับหัว entry ใน CHANGELOG · CHANGELOG มี `[0.22.0]`/`[0.23.0]` จาก release branch ครบ (ไม่มีเลขซ้ำ/ข้าม)
- [ ] gate 4 ตัวเขียวหมด: `npm test` · `npm run verify:pack` · `npm run lint:md` · check-test-count ≥ 9 — พร้อมรายงานผล
