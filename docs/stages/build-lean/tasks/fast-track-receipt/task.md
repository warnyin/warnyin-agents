...
- [ ] 1. **`src/.warnyin/workflow/triage.md`** — _ผลลัพธ์: canonical skip-list + caps + repoint_
...
- [ ] 2. **`src/.warnyin/workflow/stages/design.md`** — _ผลลัพธ์: fast path + UX detect fix; ขึ้นกับ 1 (ชี้ skip-list canonical)_
...
- [ ] 3. **สร้าง `src/.warnyin/template/stages/receipt.md`** (**นอก `[topic]/` โดยเจตนา** — กัน whole-folder copy ติดไปทุก topic) — _ผลลัพธ์: template ตาม design §3_
...
- [ ] skip-list ใน `triage.md` ตรง design `§4.1` **คำต่อคำ**
- [ ] caps อยู่ `§2D` แยก anchor จาก skip-list
- [ ] route `§2A` row fast = เวอร์ชันใหม่ (pre-flight สร้าง receipt → code-first → verify-lite → ship-lite)
- [ ] `§2C` บรรทัด why ชี้ `loop-tuning.md` (md link) — ตาราง default `§2C` **ไม่ถูกย้าย/copy** ไปไฟล์อื่น
- [ ] `stages/design.md`: exclusion เช็คก่อน signals (เจอ → จบทันที) + §7 fast row + fast path pre-flight ใน §4
- [ ] template `receipt.md` ≤40 บรรทัด อยู่ **นอก** `[topic]/`
- [ ] `installer.test.mjs` เขียว (assertion ใหม่ + เคสเดิมทั้งหมด)
- [ ] แก้เฉพาะ `src/**`
