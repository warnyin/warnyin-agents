...
- [ ] 1. **ชั้น B — `verifyInstalled` + `semverGte`** — เพิ่ม const `STAMP_MIN_VERSION = '0.17.0'` + export pure `semverGte(a,b)` (numeric tuple compare, NaN→0 defensive); แก้ branch `stamp===null` (L106-110) ตาม truth table design §4: `semverGte(expected,'0.17.0')===true → false (active)`; มิฉะนั้น → true (transition/degrade-safe). **คง guard order L96/99/106/113** (markers→!expected→stamp===null→drift)
- [ ] 2. **ชั้น A — `installViaPack` cache-bust + version-check** — (ก) เพิ่ม `env: {...process.env, npm_config_prefer_online:'true'}` ใน spawnSync ของ `npm pack` (L156 options); (ข) export pure `checkTarballVersion(extractDir, expected) → boolean`: reuse `pj` ที่ parse อยู่แล้ว (L190) → เทียบ `pj.version` กับ expected แบบ **exact-equality (trim สองฝั่ง, ไม่ใช่ semverGte)**; expected falsy → return true (ข้าม/degrade); เรียก `checkTarballVersion` ก่อน `--update` (L204) → false → `return false` (ไม่รัน --update)
- [ ] 3. **ชั้น 1 — `installViaNpx` explicit bin** — เปลี่ยน spawn args (L128) เป็น `['--yes','-p',spec,'warnyin-agents','--update']` (ระบุ `-p` + bin name แก้ scope-strip mismatch); คง `shell:isWin` + shim-detect เดิม
- [ ] 4. **unit (`src/tests/setup-dogfood.test.mjs`)** — ADD เคสตาม design §8: verifyInstalled (active 0.18.0→F, boundary 0.17.0→F, transition 0.16.0→T, non-semver 'latest'→T); semverGte (5 well-formed + 3 edge); checkTarballVersion (temp dir + fake package.json: match→T, mismatch→F, falsy→T); wire-proof (npx args array, pack prefer-online)
...
- [ ] `verifyInstalled`: stamp ขาด + expected≥0.17.0 → false; expected `'0.17.0'` พอดี → false (boundary `>=`); expected<0.17.0 → true; non-semver/falsy → true (degrade-safe); guard order คงเดิม (offline degrade ไม่ false-fail)
- [ ] `semverGte` + `checkTarballVersion` export + pure (รับ param) + unit ครอบ (รวม edge defensive)
- [ ] `installViaPack`: npm pack มี `prefer-online`; `checkTarballVersion` เรียกก่อน --update (mismatch → return false ไม่รัน)
- [ ] `installViaNpx`: args = `['--yes','-p',spec,'warnyin-agents','--update']`
- [ ] **regression:** เคสเดิมใน test ผ่านครบ (drift/degrade/CRLF/transition<0.17.0/wire-proof เดิม)
- [ ] full-gate เขียว: `node --test | check-test-count` (pass เพิ่มจาก 85) + verify-pack + lint-md
- [ ] ผ่าน `spec.md` test-flow + ทำตาม `rule.md`/`standard.md`
