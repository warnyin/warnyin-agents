...
- [x] 1. **สร้าง `src/.warnyin/workflow/loop-tuning.md`** — _ผลลัพธ์: theory canonical เดียว_
...
- [x] 2. **ใส่ pointer ในไฟล์ใหม่** — _ขึ้นกับ 1:_
...
- [x] ไฟล์ใหม่มี theory **ครบ** — diff สาระเทียบ ★ block เดิมทั้งสอง (`build.md ~64-75` / `verify.md ~58-69`) ไม่มีสาระตกหล่น (หลัก "ปรับแค่ลำดับ/การจัดกลุ่ม" + credit horizon สั้น/ยาว + เงื่อนไข + ⚠ churn + experience batching + ⚠ batch ใหญ่ + paper ref)
- [x] header ระบุ orchestrator-only + ผู้อ่าน = main loop (fix loop finding >1) ชัด
- [x] **ไม่มีตาราง default-by-tier** ในไฟล์ใหม่ — มีแค่ md link ไป `triage.md`
- [x] pointer report requirement ชี้ `stages/build.md` / `stages/verify.md` เป็น md link
- [x] **ไม่แตะไฟล์อื่นใด** — `git status` เห็นเฉพาะไฟล์ใหม่ 1 ไฟล์
- [x] md links ในไฟล์ใหม่ resolve ทั้งหมด (ยกเว้นข้อเท็จจริงที่ยอมรับ: `build.md`/`verify.md` **ยังมี ★ block เดิมอยู่** — ปกติ เพราะการลบเป็นงาน wave 2 ห้ามตีความเป็น failure ของ task นี้)
