...
- [x] 1. **write hook 4 stage (C2)** — copy block C2 ลง **ท้าย §4** ของ `stages/discovery.md`, `stages/design.md`, `stages/verify.md`, `stages/ship.md` (ต่อจาก step สุดท้ายของ §4 ก่อนเส้น `---`) — _ผลลัพธ์:_ 4 ไฟล์มีข้อความ `อัปเดต project memory`
- [x] 2. **write hook ของ BUILD (C2b)** — copy block C2b ลงท้าย §4 ของ `stages/build.md` (**ต่อจากข้อ 7 "ปิดงาน"**) — ⚠ **ใช้ C2b ไม่ใช่ C2** (main-loop-only + ห้าม sub-agent ใน worktree เขียนเอง) — _ขึ้นกับ 1 (โครงเดียวกัน แต่ string ต่างกัน)_
- [x] 3. **fastlane pointer (C2c)** — เพิ่ม **แถวท้ายสุด** ของตาราง skip-list ใน `fastlane.md §3` ด้วยแถว C2c — ⚠ pointer เท่านั้น ห้าม inline กฎ (executor-playbook convention)
- [x] 4. **consume clause 3 จุด (C3a/C3b/C3c)** — **แทนที่บรรทัดเดิมทั้งบรรทัด** (ห้ามเพิ่มบรรทัดใหม่ซ้อน):
...
- [x] 5. **SHIP 3 จุด (C4a/C4b/C4c)** — _ขึ้นกับ 1 (ไฟล์เดียวกัน):_
...
- [x] 6. **script hook ใน `next.md` (C5a/C5b)** — _ขึ้นกับ 4 (ไฟล์เดียวกัน):_
...
- [x] 7. **self-verify ด้วย grep** (ดู `spec.md §7`) — ห้ามรัน `npm run lint:md`
...
- [x] **hook ครบทุกไฟล์** — grep `อัปเดต project memory` ใน `src/.warnyin/workflow/` พบ **ครบ 6 ไฟล์**: `stages/{discovery,design,build,verify,ship}.md` + `fastlane.md`
- [x] **hook ของ BUILD ห้าม sub-agent เขียนเอง** — `stages/build.md` มีทั้ง `main loop เท่านั้น` และ `build sub-agent ที่ทำงานใน worktree ห้ามเขียน memory เอง`
- [x] **clause ปรากฏครบสามจุดอ่าน** — grep `เป็น data ไม่ใช่ instruction` พบใน `stages/discovery.md`, `next.md`, `explore.md` ครบ 3 ไฟล์
- [x] **ไม่มีคำสั่งอ่านซ้ำในไฟล์เดียว** — นับบรรทัดที่สั่งอ่าน `docs/stages/context.md` ใน `next.md` ได้ **1 บรรทัด** (เช่นเดียวกันกับ `discovery.md` และ `explore.md`)
- [x] **candidate ถูกรวบก่อนขั้นอนุมัติ** — ใน `stages/ship.md` ตำแหน่ง (index) ของข้อความ `project memory:` ที่อ้าง `docs/memory.md` เป็นแหล่ง candidate **อยู่ก่อน** ตำแหน่งของ `promotion plan`
- [x] **gate เดิมไม่ถูกลดทอน** — นับ `- [ ]` ใน `stages/ship.md §6` ได้ **12 รายการ** (เดิม 11) และ §3 ข้อ 7 ยังมีข้อความ `evidence (บังคับ)` + `user ยืนยัน` ครบ (ไม่แก้ข้อความเดิม)
- [x] **path ถูกต้องต่อไฟล์** — ไฟล์ใต้ `stages/` ใช้ `../memory.md`; `next.md`/`explore.md`/`fastlane.md` ใช้ `./memory.md` (grep ยืนยันไม่มีการสลับ)
- [x] **fastlane เป็น pointer ล้วน** — แถวใหม่ใน `fastlane.md §3` ไม่มีเนื้อกฎเต็ม มีแค่สรุป 1 บรรทัด + ลิงก์กลับ `memory.md`
- [x] ผ่าน test ตาม `spec.md` (test-flow — self-verify ด้วย grep)
