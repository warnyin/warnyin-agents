...
## 3. Sub-tasks
- [ ] 1. `resolveExpectedVersion() → string|null` — `spawnSync(npm,['view',PKG_NAME,'version'],{timeout:15000})` (`npm = isWin?'npm.cmd':'npm'`); parse บรรทัด semver จริง (`.trim().split(/\r?\n/).pop()?.trim()` + sanity `/^\d+\.\d+\.\d+/`); exit≠0/empty/timeout/ไม่ match → `null` _(ผลลัพธ์: expected version หรือ null=degrade)_
- [ ] 2. แยก `PKG_NAME = '@warnyin/agents'` จาก tag (เดิม `PKG='@warnyin/agents@latest'`); spec = `expected ? PKG_NAME+'@'+expected : PKG_NAME+'@latest'` _(ขึ้นกับ 1)_
- [ ] 3. `readStamp(root) → string|null` + ปรับ `verifyInstalled(root, expected?)` ตาม truth table `design.md §4B` (normalize trim สองฝั่ง; falsy expected = degrade; stamp ขาด = transition true; stamp≠expected = false) _(ขึ้นกับ contract §4A)_
- [ ] 4. ปรับ `installViaNpx(expected)` + `installViaPack(expected)` — รับ expected, ใช้ spec ข้อ 2, set env `npm_config_prefer_online:'true'` (npx), **ส่ง `expected` เข้า `verifyInstalled` ทั้ง 2 call site** (เดิม npx L63 + pack L126) + warn loud ตาม §4B _(ขึ้นกับ 2,3)_
- [ ] 5. `main()` — `EXPECTED = resolveExpectedVersion()` แล้วส่งเข้า `installViaNpx(EXPECTED) || installViaPack(EXPECTED)` _(ขึ้นกับ 4)_
- [ ] 6. unit test ใน `src/tests/setup-dogfood.test.mjs` — ต่อยอด 3 เคสเดิม + เคสใหม่ตาม spec test-flow
- [ ] 7. `CHANGELOG.md` — Fixed entry (false-green รอบ 2 / issue #3)
...
- [ ] `verifyInstalled(tmp)` (ไม่มี expected) → marker-only เดิม (เคส 1-3 เดิมไม่พัง — backward compat)
- [ ] `verifyInstalled(root,'9.9.9')` เมื่อ stamp=`0.1.0\n` → **false** (drift-guard — เคสแก่น)
- [ ] stamp=expected → true · stamp ขาด+expected set → true (transition) · expected `null`/`''` → true (degrade)
- [ ] CRLF: stamp `"0.16.0\r\n"` + expected `"0.16.0"` → true
- [ ] `resolveExpectedVersion` parse `"npm warn ...\n0.16.0\n"` → `0.16.0`; empty/exit≠0 → `null`
- [ ] wire-proof: `installViaNpx` **และ** `installViaPack` ส่ง `expected` เข้า `verifyInstalled` (structural assert)
- [ ] `npm test` เขียวทั้ง suite (pass-count ≥ 9); `lint:md` ผ่าน (CHANGELOG)
- [ ] ผ่าน test ตาม `spec.md` (test-flow) · ทำตาม `rule.md` + `standard.md`
