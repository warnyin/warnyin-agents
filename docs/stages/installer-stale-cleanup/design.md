...
| **C7** | **hash gate:** `source==='manifest'` ลบได้เมื่อ sha256 ของไฟล์บนดิสก์ (อ่านแล้ว `normalizeEol` ก่อน hash) **ตรง** — ไม่ตรง → ข้าม + `⚠ <path> [prune:hash-mismatch]` · `lstat().size > 5 MB` → ข้าม + `[prune:too-large]` (ใช้ `lstat` ตัวเดียวกับ C8) |
