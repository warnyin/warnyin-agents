...
| 2 | **upgrade path ถูกพิสูจน์** — black-box U1–U20 (fixture จาก `src/` จริง) | `src/tests/installer-upgrade.test.mjs` | `tasks/upgrade-path-test/` |
| 2b | **falsifiability ถูกพิสูจน์** — mutant harness + mutation matrix | `src/tests/installer-mutant.test.mjs` | `tasks/mutant-harness/` (**wave 2**) |
...
## 7. Dependency
```
wave 1 (ขนาน):  prune  │  upgrade-path-test     wave 2:  mutant-harness     wave 3:  release-hygiene
```
- **depth 3 · width 2** — ไม่มี slice ไหนแก้ไฟล์เดียวกัน
- **★ ทำไม `mutant-harness` ต้องอยู่ wave 2 (dry-run จับได้):** mutation ต้อง replace **สตริงจริงในโค้ด prune** ซึ่ง wave 1 ยังไม่มี ⇒ ถ้าเขียนพร้อมกัน `find` จะกลายเป็นคำบรรยายภาษาไทยที่ replace ไม่ติด และ **6 ใน 12 แถวพลิกเป็นเขียวไม่ได้เลยแม้ implement ถูก** เพราะ guard 6 ชั้นเป็น **อิสระต่อกัน** (ปิดชั้นเดียว ชั้นอื่นยังบล็อก เหยื่อไม่หาย) ⇒ **single-guard falsification ต้องทำที่ unit ของ slice 1 (เรียก `computeStale` ตรง ไม่ผ่าน C8)** ส่วน mutant harness ทำเฉพาะ guard ที่เป็น **ชั้นเดียวที่บล็อกเหยื่อนั้นจริง** (C4 control-char · C5 · C7 · C8 ancestor/leaf · C9 · C13)
- **★ output contract ของ wave 1 (แก้ความขัดแย้งกับ `build.md §3 ข้อ 4`):** `upgrade-path-test` รายงาน **`passed` พร้อม `expectedRed: [ชื่อเคส…]` + `redCount`** — `failed` สงวนไว้สำหรับแดงด้วย `TypeError`/`ENOENT`/spawn ล้มเท่านั้น · ถ้ารายงาน `failed` ตรง ๆ main loop จะเข้า fix loop แล้วมีโอกาสสูงที่จะไปแก้เทสหรือแก้ `cli.mjs` ให้เขียว ซึ่ง rule ห้ามไว้
- slice 2 เขียนเทสจาก contract C1–C16 (ไม่อ่านโค้ด slice 1) ⇒ **แดงตลอด wave 1 เป็นเรื่องปกติ** · **เจ้าของการทำให้เขียว = full-gate ของ BUILD (`build.md §4 step 6`, main loop)** ไม่ใช่ `release-hygiene` (wave สุดท้ายแก้ได้เฉพาะจุดเชื่อม) · `task.md` ของ slice 2 ต้องประกาศ expected-red list ระบุชื่อเคส และ acceptance คือ "แดงด้วยเหตุผลที่ถูกต้อง"
...
- **★ mutant harness (slice 2b · `src/tests/installer-mutant.test.mjs` · wave 2):** copy `cli.mjs` ลง pkg ปลอมใน temp → apply mutation ด้วย string replace → **`assert.notEqual(mutated, original)` ก่อน (fail-loud ตาม KB #33)** → spawn → assert ว่าไฟล์ที่เคส negative บอกว่า "ยังอยู่" **หายไป**
  - **★ เกณฑ์เลือกแถว: guard นั้นต้องเป็น _ชั้นเดียวที่บล็อกเหยื่อ_** — guard ที่มีชั้นอื่นบล็อกซ้ำ (เช่น `C4(3)` absolute ที่ `C4(5)` reject ก่อน · `C4(5)` ที่ C8 ซ้ำ · `C3 hash:missing` ที่ `parseManifest` reject ก่อน · `C10` ที่ design ตัด snapshot ออกแล้วจนไม่มีจุดเดียวให้ปิด · `C12` ที่เป็นเคส fs ปกติ) **ยกไปเป็น unit test ของ slice 1** (เรียก pure fn ตรง ไม่ผ่าน fs guard)
  - แถวที่ทำได้จริง: `C4(1)` backslash · `C4(2)` dot-segment · `C4(4)` control-char · `C5` agents · `C5` skills · `C7` hash · `C7` size-cap · `C8` ancestor · `C8` leaf · `C9` cap · `C13` union (ขั้วกลับ) · `C15` dry-run-ห้าม-unlink — **+ `M0` structural self-check** (`MUTATIONS.length` ตรงจำนวน และทุก anchor พบ **พอดี 1 ครั้ง**) ที่จับทั้ง anchor หาย/กำกวม และการถอดแถวออกเพื่อให้เขียว
  - **co-assertion บังคับทุกแถว:** exit 0 · มี **canary stale ที่ถูกลบสำเร็จในรันเดียวกัน** (พิสูจน์ว่า prune ทำงานอยู่จริง ไม่ใช่ไม่รันเลย) · payload ที่ยังใช้อยู่ครบ
