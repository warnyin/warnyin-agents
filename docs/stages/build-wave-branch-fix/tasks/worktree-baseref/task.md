...
- [ ] 1. `src/.warnyin/workflow/scripts/build-wave.mjs` — **(arg parse, บรรทัด ~18-20)** เพิ่ม `const baseRef = A.baseRef || null` (spec §2.1 / design §4.1) + อัปเดต comment block `args = {...}` (บรรทัด ~4-8) เพิ่ม `baseRef?` _(spec §3 / design §4.1)_
- [ ] 2. `src/.warnyin/workflow/scripts/build-wave.mjs` — **(`prompt()`, บรรทัด ~61-93)** แทรก step `0.` git merge contract **verbatim** (spec §2.2 / design §4.2) **ก่อน** บรรทัด `1. อ่านให้ครบ...` **เฉพาะ `isolate && baseRef`**; `!baseRef` → ไม่แทรก (backward compat); **ไม่ renumber** step 1-9; ใช้ `${baseRef}`/`${slug}`/`${task}` แทน placeholder _(spec §2.2 / design §4.2)_
- [ ] 3. `src/.claude/commands/warnyin/build.md` — **step 6** (บรรทัด ~15) args เพิ่ม `baseRef: "<build branch ที่สร้าง step 4>"` (spec §2.3 / design §4.3, ชื่อจริง ไม่ hardcode pattern) + **integrate note** (บรรทัด ~16) checkout เฉพาะ scoped src files + main loop อัปเดต `task.md` ตอน integrate (spec §2.4 / design §4.4) _(unify-in-place — ขยาย args + note เดิม)_
- [ ] 4. `src/.warnyin/workflow/stages/build.md` — **§3 principle 3** (บรรทัด ~31) ขยาย: worktree fork จาก **main** → agent ต้อง **sync build branch ก่อน** (กลไกใน build-wave) + **§4 step 5** (บรรทัด ~54-57) ระบุ orchestrator ส่ง `baseRef` + integrate checkout scoped — **ขยายข้อเดิม ไม่เพิ่ม principle/step ใหม่** _(design §3)_
- [ ] 5. `CHANGELOG.md` — entry `[Unreleased]` (payload reliability fix — worktree sync build branch, user-facing ตาม `docs/rule.md` §2)
- [ ] 6. **static check** — grep `baseRef` พบใน build-wave.mjs (arg parse + prompt) + build.md (step 6) · `node --check src/.warnyin/workflow/scripts/build-wave.mjs` ผ่าน · **ordering:** step `0.` ก่อนบรรทัด `1. อ่านให้ครบ` · **guard ครบ:** `|| (git merge --abort` + hard-stop "task.md ... ไม่ปรากฏ → ... failed" + `isolate && baseRef` + retry transient lock + บันทึก `notes`
- [ ] 7. `npm test` (53/53) + `npm run lint:md` + `npm run verify:pack` เขียว
...
- [ ] ทุกไฟล์แก้ตรงพิกัดตาม design §3 (แก้ 4 ไฟล์); **ไม่ renumber/ไม่สร้างกลไกขนาน** (unify-in-place) — step `0.` นำหน้า, §3 principle 3 / §4 step 5 ขยายข้อเดิม
- [ ] **contract ตรง design §4 canonical คำต่อคำ** — args §4.1, git merge contract §4.2 ฝัง verbatim ใน prompt, orchestrator arg §4.3, integrate note §4.4 (copy ไม่แต่งใหม่)
- [ ] **★ hard-stop (B2)** ใน prompt: merge สำเร็จแต่ `task.md` ไม่ปรากฏ → STOP failed ห้าม improvise — **ครบ ไม่ตัดทอน**
- [ ] **abort-on-conflict** — merge มี `|| (git merge --abort; <failed>)` (กันค้าง MERGE state) + **retry transient lock 1 ครั้ง** + **บันทึกผล merge ใน `notes`**
- [ ] **step 0 ก่อน step 1** (ordering) + **`isolate && baseRef` guard** (`!baseRef` → ไม่แทรก = backward compat; `baseRef` optional)
- [ ] **KB#11** — integrate note checkout เฉพาะ scoped `src/` files (ไม่แตะ dogfood ที่ root)
- [ ] `stages/build.md` §3 principle 3 ระบุ worktree fork จาก main → sync build branch · build.md step 6 ส่ง baseRef
- [ ] **static check ผ่าน** — grep `baseRef` (build-wave + build.md) + `node --check` + ordering + guard ครบ
- [ ] CHANGELOG `[Unreleased]` มี entry payload change
- [ ] `npm test` 53/53 + `npm run lint:md` + `npm run verify:pack` เขียว (ไม่มี regression)
