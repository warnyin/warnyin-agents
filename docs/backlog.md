...
| 2 | ~~ตัวเลขจำนวนเคสใน `docs/techstack/installer/{structure,test}.md` ค้างจากรุ่นเก่า~~ — **ทำแล้ว 2026-08-20** (commit `1670590`: วัดจริงด้วย `node --test` ต่อไฟล์ — installer.test 33/35→40 + breakdown นับใหม่จากชื่อเทสจริง · verify-pack 13→28 · พบเพิ่มนอก scope เดิม: setup-dogfood 14→32, fastlane 14→12; suite 321/321 + lint:md ผ่าน) | `ทำทีหลัง` → ทำเสร็จ 2026-08-20 | `lean-ceremony` | `dropped` — ปิดโดยทำเสร็จ |
| 3 | **cap ขนาด `spec.md` ต่อ task** ใน `validate-topic.mjs` (`CAPS` ปัจจุบันครอบ narrative ของ `design.md`/`receipt.md` แต่ **ไม่ครอบ `spec.md`**) — วัดจริง topic `installer-stale-cleanup`: `prune/spec.md` = **57,605 B ≈ 20k token** ที่ค้างใน context ของ agent ทั้ง 88 turn ⇒ **~3.1M cache-read token จากไฟล์ชุดเดียว** · **ทำระหว่าง topic นี้ไม่ได้:** gate G8 ของ `release-hygiene` รัน `validate-topic.mjs` กับ topic นี้เอง ⇒ ใส่ cap ตอนนี้ = ✖ ทันที wave 3 หยุด | `รอเงื่อนไข` — หลัง `installer-stale-cleanup` ship | `installer-stale-cleanup` | open |
| 4 | **ยุบ `rule.md` + `standard.md` ของ task เหลือไฟล์เดียว** (4 ไฟล์/task → 3) — เนื้อทับกันสูง ทั้งคู่ท่อง convention เดิมซ้ำ; วัดจริง topic นี้ task docs รวม **331 KB**, ชุดของ `prune` อย่างเดียว **105 KB ≈ 35k token** ที่ agent แบกทุก turn · ต่อยอด artifact-collapse ของ `lean-ceremony` (ที่ยุบฝั่ง BUILD/VERIFY ไปแล้ว) มาฝั่ง DESIGN · ต้องแก้ template + playbook `design.md` + `validate-topic.mjs` C1 พร้อมกัน ⇒ เป็น topic ของตัวเอง **ทำระหว่าง topic นี้ไม่ได้:** ชนกับ `MIN_PASS` ที่ `release-hygiene` กำลังคำนวณ และปน 2 topic ใน 1 release | `รอเงื่อนไข` — หลัง `installer-stale-cleanup` ship | `installer-stale-cleanup` | open |
