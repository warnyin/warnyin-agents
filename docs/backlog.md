...
| 3 | **refresh root dogfood** — `.warnyin/.warnyin-version` ยังเป็น `0.22.0` ขณะ package สูงกว่าหลายรุ่น → dogfood ที่ orchestrate จริงเก่ากว่า source | `BUILD · KB #30` | `รอเงื่อนไข` + ทำหลัง release รุ่นที่มี fix EOL แล้ว (`npx @warnyin/agents --update`) | `open` | `project-memory` |
