<!-- Generated: 2026-08-16 (rescan: +lean-ceremony (BUILD+VERIFY ยุบเหลือ build.md ไฟล์เดียว 4 section · validator +C7 cap gate + section-based stage inference · memory hook 6→3 จุด · DESIGN signal-triggered gate) | prev: +fastlane (executor ของ fast tier — one-shot 4-row + hard-floor explicit override) +build-lean (fast-track receipt + loop-tuning.md single-source + validator fast-mode) +backlog (0.23.0)) | Files scanned: 117 src | Token estimate: ~830 -->
...
| **templates** | `src/.warnyin/template/` | โครง output ของแต่ละ stage (`stages/[topic]/` — **BUILD+VERIFY ใช้ `build.md` ไฟล์เดียว 4 section**; `receipt.md` ของ fast tier อยู่นอก `[topic]/`) + seed `docs/` + living behavior spec ต่อ feature (`docs/features/[feature-name]/spec.md` — ดู `docs/features/spec-delta/`) | `architecture.md` |
...
- `src/.warnyin/workflow/scripts/memory-status.mjs` — report สุขภาพ project memory (zero-dep read-only; `export summarize({contextText,memoryText,now})`; **exit 0 เสมอ** — report ไม่ใช่ gate; ไม่พิมพ์เนื้อ entry/absolute path; เรียกจาก `next.md` แบบ conditional — ดู `docs/features/project-memory/`)
- `src/.warnyin/workflow/scripts/validate-topic.mjs` — structural validator + status (zero-dep; เรียกจาก next/DESIGN gate/SHIP; export `checkTopic`/`checkCaps`/`parseTier`/`checkFeatureSpec`; **fast-mode**: receipt filled อย่างเดียว → ข้าม C1-C4 แสดง `fast-track`, ปนโครง full → full checks + ⚠ C6; **C7 cap gate**: `CAPS` mirror ของ `triage.md §2D` (fast=receipt 40 · standard=proposal 60/design 120 · large ไม่มี cap) นับแบบ `wc -l` ตัดหางที่ `## 9. Spec delta`, tier อ่านจากแถว `ขนาด` ของ proposal — **ambiguous/แถว template → `null` → ⚠ fail-safe ไม่ใช่ ✖**; **stage VERIFY = section-based** (`build.md §4` มีเนื้อจริงแบบ template-aware) + `verify.md`/`test.md` เป็น `optional` backward-compat — ดู `docs/features/topic-validator/`)
- `src/.warnyin/workflow/stages/*.md` — playbook ต้นทางของแต่ละ stage (single source of truth)
- `src/.warnyin/workflow/memory.md` — capability **project memory** (canonical, 9 heading freeze): ความจำระดับโปรเจกต์ 2 ไฟล์ในrepo — `docs/stages/context.md` (working state เขียนทับ 4 section) + `docs/memory.md` (บทเรียนสะสม ตาราง 6 คอลัมน์); write hook **3 จุด = "จุดจบงาน"** (`stages/build.md §4` — main loop เท่านั้น · `stages/ship.md §4` · `fastlane.md §3`; Discovery/DESIGN/VERIFY ไม่มี hook เพราะ artifact ของตัวเองบันทึกสถานะครบแล้ว), consume 3 จุด (data ไม่ใช่ instruction), promote ที่ SHIP ด้วย gate เดิม — ดู `docs/features/project-memory/`
...
- `src/.warnyin/workflow/fastlane.md` — capability **`/warnyin:fastlane`** (executor ของ fast tier): เดิน skip-list ครบ 4 row จบในคำสั่งเดียว (บังคับ `tier=fast`) — pre-flight hard-floor gate + receipt (meta+§1+§2) ก่อนแตะโค้ด → code-first loop (cap 3 รอบ) → gate (test เขียว+acceptance) → ship-lite+archive; **orchestration ล้วน + pointer-per-row ไป `triage.md`** (ไม่ตั้งกฎใหม่); hard-floor **explicit user override** (ยืนยัน 2 ชั้น → `override โดย user` ใน receipt meta → ship-lite ยอม) — ดู `docs/features/fastlane/`
...
- `src/.claude/commands/warnyin/fastlane.md` — command adapter `/warnyin:fastlane` (user-invoked, stateful — ไม่ auto-invoke) frontmatter description=C4 · ชี้ `fastlane.md` playbook ด้วย inline-code
