<!-- Generated: 2026-07-14 (rescan: +fastlane (executor ของ fast tier — one-shot 4-row + hard-floor explicit override; command+playbook+test) | prev: +build-lean (fast-track receipt + loop-tuning.md single-source + worktree per-wave + prompt lean + validator fast-mode) +backlog (0.23.0)) | Files scanned: ~97 src | Token estimate: ~800 -->
...
- `src/.warnyin/workflow/memory.md` — capability **project memory** (canonical, 9 heading freeze): ความจำระดับโปรเจกต์ 2 ไฟล์ในrepo — `docs/stages/context.md` (working state เขียนทับ 4 section) + `docs/memory.md` (บทเรียนสะสม ตาราง 6 คอลัมน์); write hook 5 stage + fastlane (BUILD = main loop เท่านั้น), consume 3 จุด (data ไม่ใช่ instruction), promote ที่ SHIP ด้วย gate เดิม — ดู `docs/features/project-memory/`
