...
- **★ อาการที่ 2 ของรากเดียวกัน — assertion กำพร้า (topic `lean-ceremony`):** เมื่อ **หลาย slice ลบ/เพิ่มสมาชิกของเซตคนละไฟล์** จะไม่มี slice ไหนทำให้เทสเขียวได้เลย ⇒ เทสแดงตลอด wave และ task file ของแต่ละ slice ชี้เจ้าของการอัปเดต expected ไปคนละทาง (ที่นี่ 3 task ชี้ไม่ตรงกัน และ task ที่ถูกชี้มี rule ห้ามแตะเทส) — **วิธีแก้:** มอบ ownership ให้ **wave สุดท้าย** พร้อมเงื่อนไข "พิสูจน์ด้วย negative-grep ว่าสถานะจริงเปลี่ยนตาม contract ก่อน จึงแก้ expected" และประกาศใน `design.md` + task file ทุกใบว่าเทสนี้จะแดงตลอด wave แรกเป็นเรื่องปกติ — **ป้องกันซ้ำ:** coherence review หลังแตก task ต้องถาม "เทสตัวไหนที่ไม่มี slice เดียวทำให้เขียวได้?" (`docs/rule.md §2`)
...
- **★ เจอซ้ำใน topic `lean-ceremony` — กฎข้างบนถูกละเมิดอีกครั้ง จึงยกระดับ:** แก้ stage inference ให้ "§4 ต้องมีเนื้อจริง" แล้วเขียน fixture เองเป็น §4 ที่มีแต่ blockquote — แต่ §4 ของ template จริงมี table meta / `###` ย่อย / checkbox / data row placeholder รวม 28 บรรทัดที่ผ่านเงื่อนไข ⇒ **เทสเขียวแต่ของจริงยังหลุด 100%** และเจตนาของการแก้ไม่บรรลุเลย · **ขยายกฎ:** ไม่ใช่แค่ parser — **heuristic ทุกตัวที่ตัดสินว่า "artifact เริ่มเติมหรือยัง" ต้องมีเคสที่ fixture = ไฟล์ template ที่ shipped จริง ทั้งขั้ว "ยังไม่เติม" และ "เติมแล้ว"**; fixture ที่ผู้เขียนเทสแต่งขึ้นพิสูจน์ได้แค่ว่าโค้ดตรงกับจินตนาการของผู้เขียน
...
### 34. parser อ่าน "ค่าตัวอย่างใน template" เป็นค่าจริง → gate ปิดตัวเองเงียบทั้งโปรเจกต์
- **อาการ:** validator รายงาน `✓ โครงครบ` exit 0 ทั้งที่เอกสารยาวเกิน cap และผู้ใช้ยังไม่เคยกรอก tier — ไม่มีทั้ง ✖ และ ⚠ (เงียบสนิท ทุก gate เขียว)
- **Root cause:** เช็คใหม่อ่าน tier จากแถวตาราง `| **ขนาด** | ... |` ด้วย regex เอา match แรก — แถวของ **template** คือ `` `fast` / `standard` / `large` (…) `` จึงคืน `fast` เสมอเมื่อยังไม่เติม; และเพราะ tier ของ `fast` ครอบแค่ `receipt.md` ไฟล์อื่นจึงไม่ถูกเช็ค **และค่าที่ได้ไม่ใช่ `null` จึงไม่เข้าเส้น fail-safe ที่เอกสารประกาศไว้** — "default state ของ template" กลายเป็นค่าที่ถูกต้องโดยบังเอิญ
- **วิธีแก้:** parse เฉพาะ **cell ค่า** (หลัง pipe ตัวที่ 2) · ให้ค่าที่อยู่ใน backtick มาก่อน · เจอ keyword ต่างชนิด >1 ตัว = **ambiguous → คืน `null`** เข้าเส้น fail-safe; ตรวจ regression กับ artifact จริงทุกใบก่อนสรุป (ที่นี่ 38 ใบ — พบว่ากฎ "ambiguous ทั้งบรรทัด" แบบดิบจะพัง proposal ที่เขียนอธิบายก้ำกึ่ง เช่น `` `standard` (ก้ำกึ่ง fast/standard → ปัดขึ้น) ``)
- **ป้องกันซ้ำ:** parser ทุกตัวที่อ่านค่าจาก artifact **ต้องมีเคสที่ป้อนไฟล์ template ที่ยังไม่เติม** — ถ้าคืนค่าที่ "ดูใช้ได้" จากโครงเปล่า แปลว่า gate จะเงียบทั้งโปรเจกต์ (คู่กับ #32); และ **fail-safe ที่เอกสารประกาศต้องมีเทสพิสูจน์ว่ามันถูกเรียกจริง** ไม่ใช่แค่มีโค้ดอยู่

### 35. เอกสาร narrative ผิดจากงานจริงโดยที่ gate ทุกตัวเขียว
- **อาการ:** `npm test` / `lint:md` / `verify:pack` / validator เขียวหมด แต่ CHANGELOG บรรยายพฤติกรรมที่ไม่มีอยู่จริง (อ้าง signal ที่ไม่มีในโค้ด · บอกว่า auto-route ไปอีก stage หนึ่งทั้งที่จริงคือ executor คนละตัว · เขียนว่า "ไม่ต้องเดิน VERIFY loop เพิ่ม" ซึ่งขัดกับกฎผู้ตรวจอิสระ) และ runbook ยกตัวอย่าง error string ที่ไม่เคยถูกพิมพ์ + คำสั่ง `grep`/`sed` ที่รันแล้วได้ผลผิด
- **Root cause:** gate ทุกตัวของ repo ตรวจ **โครงสร้าง** (ไฟล์/ลิงก์/เทส) ไม่มีตัวไหนตรวจ **ความตรงของเนื้อความเทียบ source**; ยิ่งเสี่ยงเมื่อให้ agent เขียนสรุปงานที่ตัวเองไม่ได้ลงมือทุกส่วน
- **วิธีแก้:** main loop เขียน section narrative เองใหม่ทั้งก้อน (single-writer) + ผู้ตรวจอิสระไล่ทุก claim เทียบไฟล์จริง **และรันทุกคำสั่งที่เขียนในเอกสาร** — รอบสองยังจับเพิ่มได้อีก 3 จุดหลังรอบแรกประกาศว่าแก้แล้ว (ตัวเลข evidence วัดคนละกติกากับ gate · `awk` คลาด +1 · error string ไม่ตรง)
- **ป้องกันซ้ำ:** ดู `docs/rule.md §5` — คำสั่งในเอกสารต้องรันจริงก่อน commit · error string ต้อง copy จาก output จริง · ตัวเลข evidence ต้องวัดด้วย **กติกาเดียวกับ gate ที่ ship จริง**
