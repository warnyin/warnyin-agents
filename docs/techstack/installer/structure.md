...
src/tests/validate-topic.test.mjs unit + executable ของ validator (75 เคส: C1-C7 · cap boundary =cap/cap+1 · stage D1-D9 ใช้ template จริงเป็น fixture · exe exit code)
...
src/.warnyin/workflow/scripts/validate-topic.mjs structural validator ของ topic (read-only; C1-C7 + stage inference; export checkTopic/checkCaps/parseTier/checkFeatureSpec)
src/.warnyin/template/stages/[topic]/build.md  **artifact เดียวของ BUILD+VERIFY** (4 section: §1 ผล build · §2 gate · §3 แผนเทส · §4 ผล verify) — ไม่มี test.md/verify.md แล้ว (topic เก่าที่ยังมีสองไฟล์นั้น = backward-compat, validator infer VERIFY ได้ทั้งสองทาง)
