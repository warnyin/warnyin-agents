...
- **build-orchestration: commit topic docs ก่อน fan-out** — BUILD orchestrator ต้อง commit `docs/stages/<slug>/` ลง build branch **ก่อน** fan-out worktree agent และให้ worktree branch จาก build branch (ไม่ใช่ main) — agent จะเห็น task ของตัวเองครบ; สถานะ/บันทึกใน topic dir ที่ agent แก้ไม่ได้จาก worktree → **main loop อัปเดตตอน integrate** (ห้ามถือว่า task ล้ม) — evidence: topic `feature-spec-delta` troubleshooting TS-1 (เจอซ้ำ 2/2 task ใน wave เดียว) + `validator-status` TS-2 (เจอซ้ำ wave 2)
